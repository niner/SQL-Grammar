...
#        ( <.WSP>**{0..$*indent} )
            :my @*current-rule-dependencies = Array.new;
...
# RFC 5234 errata ID 2968
    ...
    method parse(|c) {
        ...
        my @*current-rule-dependencies;
        my @*classes;
        my $*in-option = False;
        my %hmod = c.hash;
        %hmod<name>:delete;
        %hmod<actions> = Grammar::ABNF::GrammarGenerator unless %hmod<actions>:exists;
        my \cmod = \(|c.list, |%hmod);
        nextwith(|cmod);
    }
...
class Grammar::ABNF::Slang-actions is Grammar::ABNF::GrammarGenerator { }
